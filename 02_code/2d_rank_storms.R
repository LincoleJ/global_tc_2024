...
admin2_units <- load_adm2_who_mapping()
adm2_key = admin2_units %>%
  distinct() %>% 
  select(shapeID, shapeName, country)
year = 2024
...
adm2_pop = left_join(adm2_pop, adm2_key, by = c("shapeID" = "shapeID"))
...
tc_pday_by_storm = person_day_tc_exposure %>% group_by(storm_id) %>%
  summarise(total_pday = sum(total_person_day_exposure),
            countries_affected = paste(unique(country), collapse = ", "),
            .groups = "drop")
...
# Add country affected to the datasets, and save the summarized storm data
